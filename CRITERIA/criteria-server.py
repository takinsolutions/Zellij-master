...
def superClass(ontology, prefix, baseURL):
        g = Graph()
        ontologyPath = os.path.join(os.path.dirname(os.path.realpath(__file__)), "src/ontologies", ontology)
...
def get_instance(rdfInputHandle, inFormat="turtle"):
        ...
        out = StringIO()
        mmdFile = os.path.join(os.path.dirname(os.path.realpath(__file__)), "./src/templates/instance.mmd")
...
def get_ontology(rdfInputHandle, inFormat="turtle"):
        if isinstance(rdfInputHandle, str):
                rdfInputHandle = StringIO(rdfInputHandle)
        out = StringIO()
        mmdFile = os.path.join(os.path.dirname(os.path.realpath(__file__)), "./src/templates/ontology.mmd")
        template = open(mmdFile, "r", encoding="utf-8")
        read = template.read()
        out.write(read)
